...
async function provision() {
  const response = await fetch(`${URL}/functions/v1/provision-dealer`, {
    method: 'POST',
    headers: { apikey: ANON, Authorization: `Bearer ${founderToken}`, 'Content-Type': 'application/json', Origin: 'https://mapco-navy.vercel.app', 'X-Idempotency-Key': `founder-e2e:${run}:${randomUUID()}` },
    body: JSON.stringify({
...
async function resolveBuyer(value) {
  const response = await fetch(`${URL}/functions/v1/resolve-client-link`, {
    method: 'POST', headers: { apikey: ANON, Authorization: `Bearer ${ANON}`, 'Content-Type': 'application/json', Origin: 'https://mapco-navy.vercel.app', 'x-mapco-client': 'v2-web' }, body: JSON.stringify({ token: value }),
  });
...
try {
  ...
  check(resolved.response.ok && resolved.body?.link?.properties?.[0]?.placement?.mapId === mapId,
    'buyer resolver returns the saved map placement');
