...
const curation = JSON.parse(readFileSync(join(HERE, 'map-curation.json'), 'utf8'));
const baseline = JSON.parse(readFileSync(join(REPO, 'docs/maps/compatibility-baseline.json'), 'utf8'));
 ...
 */
const OVERRIDES = {
  // GMADA is the authority; this image covers only Aerocity blocks E/F.
  'gamada aerocity mohali': { kind: 'PROJECT_MAP', city: 'Aerocity', why: 'GMADA Aerocity partial layout (blocks E/F)' },
...
for (const { file, full, folder } of entries) {
  ...
  const curatedSource = curation.sources[[folder, file].filter(Boolean).join('/')];
  const classified = curatedSource ?? classify(base, folder);
  ...
  const id = curatedSource?.id ?? norm(base).replace(/\s+/g, '-');
  if (seen.has(id)) {
    throw new Error(`Duplicate map ID ${id}; curate an explicit stable ID instead of silently renumbering.`);
  }
  seen.add(id);
  ...
  const oldMap = baseline.maps.find(m => m.id === id);
  if (oldMap && oldMap.sha256 !== hash) throw new Error(`Existing raster bytes changed: ${id}`);
  if (!oldMap && (imageHashes.has(hash) || baseline.maps.some(m => m.sha256 === hash))) {
    throw new Error(`Duplicate image for new map ${id}`);
  }
  imageHashes.set(hash,id);
  ...
  maps.push({
    ...
    active: true,
    ...(curatedSource ? { name: curatedSource.name } : {}),
    ...(curation.byId[id] ?? {}),
  });
}

// Keep existing resolution order and never overwrite a persisted pin's coordinate space.
const oldOrder = new Map(baseline.maps.map((m, i) => [m.id, i]));
maps.sort((a,b) => (oldOrder.get(a.id) ?? Infinity) - (oldOrder.get(b.id) ?? Infinity) || a.id.localeCompare(b.id));
for (const old of baseline.maps) {
  const next = maps.find(m => m.id === old.id);
  if (!next || next.image !== old.image || JSON.stringify(next.dimensions) !== JSON.stringify(old.dimensions)) {
    throw new Error(`Existing placement coordinate space changed: ${old.id}`);
  }
}
{
  const module = [
  ...
  const index = `${JSON.stringify({ version: 1, count: maps.length, maps }, null, 2)}\n`;
  if (checkOnly) {
    if (readFileSync(REGISTRY_OUT,'utf8').replace(/\r\n/g,'\n') !== module
      || readFileSync(join(OUT_DIR,'index.json'),'utf8').replace(/\r\n/g,'\n') !== index) {
      throw new Error('Generated map registry is stale. Run the importer.');
    }
    for (const m of maps) {
      const path = join(V2,'public',m.image);
      if (!existsSync(path)) throw new Error(`Missing map asset: ${m.image}`);
      const expected = [...imageHashes.entries()].find(([,id]) => id === m.id)?.[0];
      const actual = createHash('sha256').update(readFileSync(path)).digest('hex');
      // Existing byte-duplicate IDs are preserved for compatibility.
      const old = baseline.maps.find(old => old.id === m.id);
      if (actual !== (expected ?? old?.sha256)) throw new Error(`Published raster differs from source: ${m.id}`);
    }
  } else {
    writeFileSync(REGISTRY_OUT, module);
    writeFileSync(join(OUT_DIR, 'index.json'), index);
  }
}
