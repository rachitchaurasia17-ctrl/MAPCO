...
<div class="pm-landing" style="position:fixed;inset:0;overflow:hidden;background:#fff4de;font-family:var(--pm-font-ui);display:flex;flex-direction:column">
  ...
  <div style="position:relative;z-index:2;flex:1;min-height:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:clamp(14px,2.8vh,28px);padding:0 clamp(20px,3vw,40px) clamp(14px,2.2vh,24px);margin-top:clamp(-30px,-4.5vh,-50px)">

    <div style="display:flex;flex-direction:column;align-items:center;text-align:center;max-width:640px;margin-bottom:clamp(16px,2.5vh,32px)">
      <img src="/assets/mapco-logo.png" style="width:clamp(150px,22vh,260px);height:auto;mix-blend-mode:multiply;margin-bottom:clamp(-25px,-3.5vh,-40px);animation:lRise .95s cubic-bezier(.2,.8,.2,1) both">
      <h1 style="margin:0;font-family:var(--pm-font-display);font-weight:500;font-size:clamp(56px,11vh,120px);line-height:.9;letter-spacing:-.035em;color:#16281e;animation:lRise 1s cubic-bezier(.2,.8,.2,1) both;animation-delay:.08s">MAPCO</h1>
      <div style="margin-top:clamp(8px,1.4vh,14px);font-size:clamp(10px,1.3vh,13px);font-weight:800;letter-spacing:.34em;text-transform:uppercase;color:#a8792a;animation:lWide 1.4s cubic-bezier(.2,.8,.2,1) both;animation-delay:.3s">Property Presentation System</div>
    </div>
    ...
    <div class="pm-land-cards" style="display:flex;gap:clamp(12px,1.4vw,20px);width:100%;max-width:1160px;flex:0 1 auto;min-height:0">
      ...
      <a href="/admin/owner.html" class="pm-land-card-1" style="flex:1;min-width:0;display:flex;flex-direction:column;overflow:hidden;border-radius:clamp(18px,2.4vh,26px);background:#fffaf0;border:1px solid #f2dca6;box-shadow:0 2px 3px rgba(40,30,10,.04),0 30px 56px -38px rgba(120,86,10,.8);color:inherit;text-decoration:none;transition:transform .38s cubic-bezier(.2,.8,.2,1),box-shadow .38s ease,border-color .3s ease;animation:lRise .95s cubic-bezier(.2,.8,.2,1) both;animation-delay:.2s">
        <div style="position:relative;flex:none;height:clamp(84px,13vh,120px);background:#ffc93c;background-image:radial-gradient(120% 130% at 12% 8%,#ffe28a,#f7b21f 62%,#e79a0c);overflow:hidden">
          ...
          <div style="position:absolute;left:clamp(16px,2vw,22px);top:50%;transform:translateY(-50%);width:clamp(42px,6vh,54px);height:clamp(42px,6vh,54px);border-radius:16px;background:#1a2f24;color:#ffd75e;display:grid;place-items:center;box-shadow:0 14px 26px -14px rgba(26,47,36,.8)"><i class="ph-fill ph-chart-donut" style="font-size:clamp(21px,3vh,27px)"></i></div>
        ...
        <div style="padding:clamp(14px,2.2vh,22px) clamp(16px,2vw,24px) clamp(16px,2.4vh,24px);display:flex;flex-direction:column;flex:1;min-height:0">
          <div style="font-size:clamp(10px,1.3vh,11.5px);font-weight:800;letter-spacing:.16em;text-transform:uppercase;color:#a8792a">For you</div>
          <div style="margin-top:6px;font-family:var(--pm-font-display);font-weight:600;font-size:clamp(26px,4vh,36px);line-height:1.05;letter-spacing:-.01em;color:#241f1c">MAPCO <span style="color:#a8792a">DESK</span></div>
          <div style="display:flex;align-items:center;gap:8px;margin-top:auto;padding-top:clamp(12px,1.8vh,20px);font-size:clamp(14px,1.8vh,15.5px);font-weight:800;color:#8a5a0c">Enter command <i class="ph-bold ph-arrow-right" style="font-size:15px;animation:lSlide 2.6s ease-in-out infinite"></i></div>
        </div>
      </a>
      ...
      <a href="/admin/marketing.html" class="pm-land-card-3" style="flex:1;min-width:0;display:flex;flex-direction:column;overflow:hidden;border-radius:clamp(18px,2.4vh,26px);background:#fffaf0;border:1px solid #ded1f4;box-shadow:0 2px 3px rgba(40,30,10,.04),0 30px 56px -38px rgba(70,40,150,.7);color:inherit;text-decoration:none;transition:transform .38s cubic-bezier(.2,.8,.2,1),box-shadow .38s ease,border-color .3s ease;animation:lRise .95s cubic-bezier(.2,.8,.2,1) both;animation-delay:.36s">
        <div style="position:relative;flex:none;height:clamp(84px,13vh,120px);background:#5b32c4;background-image:radial-gradient(120% 130% at 18% 4%,#a983f5,#6a3ed6 58%,#4a26a8);overflow:hidden">
          ...
          <div style="position:absolute;left:clamp(16px,2vw,22px);top:50%;transform:translateY(-50%);width:clamp(42px,6vh,54px);height:clamp(42px,6vh,54px);border-radius:16px;background:#ffe1e6;color:#5b32c4;display:grid;place-items:center;box-shadow:0 14px 26px -14px rgba(40,20,90,.7)"><i class="ph-fill ph-megaphone-simple" style="font-size:clamp(21px,3vh,27px)"></i></div>
        ...
        <div style="padding:clamp(14px,2.2vh,22px) clamp(16px,2vw,24px) clamp(16px,2.4vh,24px);display:flex;flex-direction:column;flex:1;min-height:0">
          <div style="font-size:clamp(10px,1.3vh,11.5px);font-weight:800;letter-spacing:.16em;text-transform:uppercase;color:#5b32c4">For Marketing</div>
          <div style="margin-top:6px;font-family:var(--pm-font-display);font-weight:600;font-size:clamp(26px,4vh,36px);line-height:1.05;letter-spacing:-.01em;color:#241f1c">MAPCO <span style="color:#5b32c4">MARKETING</span></div>
          <div style="display:flex;align-items:center;gap:8px;margin-top:auto;padding-top:clamp(12px,1.8vh,20px);font-size:clamp(14px,1.8vh,15.5px);font-weight:800;color:#5b32c4">Open marketing <i class="ph-bold ph-arrow-right" style="font-size:15px;animation:lSlide 2.6s ease-in-out infinite;animation-delay:.6s"></i></div>
        </div>
      </a>
