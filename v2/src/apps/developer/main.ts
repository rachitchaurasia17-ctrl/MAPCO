...
import { isFounderCredentialCallback, mountFounderCredentialSetup } from './founder-auth';

const app = document.getElementById('app');
const credentialCallback = isFounderCredentialCallback();
if (app) void (async () => {
  if (await mountFounderCredentialSetup(app, credentialCallback)) return;
  await requireSession(app, async () => {
    try {
