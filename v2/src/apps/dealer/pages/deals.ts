...
<div style="max-width:1080px;margin:0 auto;padding:34px 40px 70px">
  <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:20px;flex-wrap:wrap;animation:omRise .5s cubic-bezier(.2,.8,.2,1) both">
    <div>
      <h1 style="margin:0;font-family:'Newsreader',serif;font-weight:500;font-size:34px;letter-spacing:-.015em;color:#241f1c">My Deals</h1>
      <p style="margin:8px 0 0;font-size:17px;color:#6b6156">Your deal book — name each deal your way, link the plot, keep your money in view.</p>
    </div>
...
export async function renderDeals(el: HTMLElement): Promise<void> {
  ...
  function dealCard(d: Deal): string {
    ...
    return `
    <div style="display:flex;align-items:center;gap:16px;padding:20px 22px;border-radius:18px;background:${st.card};border:1px solid ${st.border};box-shadow:0 1px 2px rgba(30,28,22,.03),0 14px 34px -26px rgba(30,28,22,.6);cursor:pointer;transition:transform .12s" onmouseenter="this.style.transform='translateY(-2px)'" onmouseleave="this.style.transform='none'">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:16px;font-weight:800;color:#2f2a2d;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(d.name)}</div>
        <div style="font-size:13px;color:#8d8271;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:1px">${esc(d.client)} · ${esc(d.propSub)}</div>
        <div style="display:flex;align-items:center;gap:9px;margin-top:9px;flex-wrap:wrap">
          <span style="font-family:'Newsreader',serif;font-weight:600;font-size:19px;color:#241f1c">${formatINR(d.value)}</span>
    ...
    el.innerHTML = shell(`
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:24px;animation:omRise .55s cubic-bezier(.2,.8,.2,1) both;animation-delay:.06s">
        <div style="background:#ffc93c;background-image:linear-gradient(135deg,#ffdc7a,#f4ae14);border-radius:20px;padding:24px 26px;color:#1f1a12">
          ...
          <div style="font-family:'Newsreader',serif;font-weight:500;font-size:46px;line-height:1;color:#1f1a12;margin-top:8px">${formatINR(pipeline)}</div>
        </div>
        <div style="background:#d9f5e3;border:1px solid #a6e3c0;border-radius:20px;padding:24px 26px">
          ...
          <div style="font-family:'Newsreader',serif;font-weight:500;font-size:46px;line-height:1;color:#0b8f45;margin-top:8px">${formatINR(comm)}</div>
        </div>
      </div>
      ...
      <div style="background:#faf7ff;border:1px solid #e4dbf7;border-radius:22px;overflow:hidden;box-shadow:0 1px 2px rgba(30,28,22,.03),0 14px 34px -26px rgba(30,28,22,.6)">
        ${done.map(d => {
          const st = STAGE_STYLE[d.stage];
          return `<div style="display:flex;align-items:center;gap:16px;padding:16px 22px;border-bottom:1px solid #f6e8c8;cursor:pointer;transition:background .12s" onmouseenter="this.style.background='#fdfbff'" onmouseleave="this.style.background='transparent'">
            ...
            <div style="flex:1;min-width:0"><div style="font-size:16px;font-weight:800;color:#2f2a2d">${esc(d.name)}</div><div style="font-size:13px;color:#8d8271;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(d.client)} · ${esc(d.propSub)}</div></div>
            <div style="font-family:'Newsreader',serif;font-weight:600;font-size:20px;color:#241f1c;flex:none">${formatINR(d.value)}</div>
